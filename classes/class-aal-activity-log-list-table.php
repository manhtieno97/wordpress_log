...
class AAL_Activity_Log_List_Table extends WP_List_Table {
	...
	public function extra_tablenav( $which ) {
		...
		if ( $users || $types ) {
			...
			$max = date("Y-m-d");
			echo '<input type="date" max='.$max.' name="dateFrom" style="float:left" value="'.$dateFrom.'" />';
			echo '<input type="date" max='.$max.' name="dateTo" style="float:left" value="'.$dateTo.'" />';
	...
	public function prepare_items() {
		...
		$where                 = ' WHERE 1 = 1';
        $_REQUEST['typeshow'] = 'Post';
        $_REQUEST['subtypeshow'] = 'product';
        ...
        if ( !empty( $_REQUEST['dateFrom']) && empty( $_REQUEST['dateTo'] ) ) {
            ...
            $start_time = mktime( 0, 0, 0, date( 'm', $dateFrom ), date( 'd', $dateFrom ), date( 'Y', $dateFrom ) );;
            $end_time = mktime( 23, 59, 59, date( 'm', $dateFrom ), date( 'd', $dateFrom ), date( 'Y', $dateFrom ) );
        ...
        if ( empty( $_REQUEST['dateFrom']) && !empty( $_REQUEST['dateTo'] ) ) {
            ...
            $start_time = mktime( 0, 0, 0, date( 'm', $dateTo ), date( 'd', $dateTo ), date( 'Y', $dateTo ) );;
            $end_time = mktime( 23, 59, 59, date( 'm', $dateTo ), date( 'd', $dateTo ), date( 'Y', $dateTo ) );
        ...
        {
            if(!empty($item->object_id) && !in_array($item->object_id,$object)){
                $object[] = $item->object_id;
            }else{
                unset($this->items[$key]);
            }
            if(!empty($_REQUEST['usershow']) && $item->user_id != $_REQUEST['usershow']){
                unset($this->items[$key]);
            }
