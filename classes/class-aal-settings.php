...
class AAL_Settings {
	...
	protected $title = 'Cấu hình thống kê';
    protected $note = 'Tùy chọn hiển thị';
    protected $log_time = 'Thời gian lưu trữ';
    protected $day_time = 'ngày.';
    protected $desc_time = 'Số ngày tối đa để giữ thống kê hoạt động. Để trống để giữ thống kê hoạt động mãi mãi (không được khuyến nghị).';

    protected $reset_title = 'Xóa thông kê lịch sử';
    protected $reset_action = 'Xóa dữ liệu cũ';
    protected $reset_warning = 'Cảnh báo: Nhấp chuột vào đây sẽ xóa hết tất cả thống kê lịch sử trở về trước!';
	...
	public function action_admin_menu() {
		$this->hook = add_submenu_page(
			'activity_log_page',
			__( $this->title, 'aryo-activity-log' ), 	// <title> tag
			__( $this->title, 'aryo-activity-log' ), 			// menu label
		...
		switch ( $section ) {
			case 'general':
				...
				add_settings_section(
					...
					__( $this->note, 'aryo-activity-log' ),	// Title to be displayed on the administration page
				...
				add_settings_field(
					'logs_lifespan',
					__( $this->log_time, 'aryo-activity-log' ),
					...
					array(
						...
						'type'    => 'number',
						'sub_desc'    => __( $this->day_time, 'aryo-activity-log' ),
						'desc'    => __( $this->desc_time, 'aryo-activity-log' ),
					)
				);
				...
				if ( apply_filters( 'aal_allow_option_erase_logs', true ) ) {
					add_settings_field(
						...
						__( $this->reset_action, 'aryo-activity-log' ),
						array( 'AAL_Settings_Fields', 'raw_html' ),
						$this->slug,
						'general_settings_section',
						array(
							'html' => sprintf( __( '<a href="%s" id="%s">'.$this->reset_action.'</a>', 'aryo-activity-log' ), add_query_arg( array(
								'action' => 'aal_reset_items',
								'_nonce' => wp_create_nonce( 'aal_reset_items' ),
							), admin_url( 'admin-ajax.php' ) ), 'aal-delete-log-activities' ),
							'desc' => __( $this->reset_warning, 'aryo-activity-log' ),
						)
					);
				}
				break;
	...
	public function display_settings_page() {
		...
		<div class="wrap">
			...
			<h1 class="aal-page-title"><?php _e( $this->title, 'aryo-activity-log' ); ?></h1>
			<?php settings_errors(); ?>
...
final class AAL_Settings_Fields {
	...
	public static function general_settings_section_header() {
		?>
		<p><?php _e( 'Đây là một số cài đặt cơ bản cho Cấu hình thống kê', 'aryo-activity-log' ); ?></p>
		<?php
	}
